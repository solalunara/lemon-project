...
void CTriggerTeleport::Touch( CBaseEntity *pOther )
{
	...
	if (!pentLandmark && !HasSpawnFlags(SF_TELEPORT_PRESERVE_ANGLES) )
	{
		...
		pVelocity = &vecZero;
#else
		pVelocity = vec3_origin;	//BUGBUG - This does not set the player's velocity to zero!!!
#endif
	}

	tmp += vecLandmarkOffset;

	CBasePlayer *pOtherAsPlayer = dynamic_cast<CBasePlayer *>( pOther );
	if ( pOtherAsPlayer )
	{
		if ( pAngles )
			pOtherAsPlayer->pl.v_angle = QAngle( pAngles->x, pAngles->y, pAngles->z );
		pOtherAsPlayer->pl.fixangle = FIXANGLE_ABSOLUTE;
		pOtherAsPlayer->UpdateVPhysicsPosition( tmp, pVelocity, 0.0f );
		pOtherAsPlayer->Teleport( &tmp, pAngles, &pVelocity );
	}
	else
