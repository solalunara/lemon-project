...
ConVar sv_stop_decrease_blast_speed( "-minusblastspeed", "0", FCVAR_NONE, "stops decreasing blast speed" );
...
void CWeaponPortalgun::Spawn( void )
{
	...
	sv_increase_blast_speed.SetValue( false );
	sv_decrease_blast_speed.SetValue( false );
...
void CWeaponPortalgun::Think( void )
{
	...
	if ( sv_stop_increase_blast_speed.GetBool() )
		sv_increase_blast_speed.SetValue( 0 );
	if ( sv_stop_decrease_blast_speed.GetBool() )
		sv_decrease_blast_speed.SetValue( 0 );

	sv_stop_increase_blast_speed.SetValue( 0 );
	sv_stop_decrease_blast_speed.SetValue( 0 );

	if ( sv_increase_blast_speed.GetBool() )
		sv_portal_blast_speed.SetValue( sv_portal_blast_speed.GetFloat() + 10.0f );
	if ( sv_decrease_blast_speed.GetBool() )
		sv_portal_blast_speed.SetValue( sv_portal_blast_speed.GetFloat() - 10.0f );
...
float CWeaponPortalgun::TraceFirePortal( bool bPortal2, const Vector &vTraceStart, const Vector &vDirection, trace_t &tr, Vector &vRelativeVel, QAngle &qRelativeVel, Vector &vFinalPosition, QAngle &qFinalAngles, int iPlacedBy, bool bTest /*= false*/ )
{
	CTraceFilterSimpleClassnameList baseFilter( this, COLLISION_GROUP_PLAYER );
	...
	// default is mask_all for some reason, changing to mask_shot_portal
	UTIL_TraceLine( vTraceStart, vTraceStart + (vDirection * m_fMaxRange1), MASK_SHOT_PORTAL, &baseFilter, &tr );
	...
	if ( tr.m_pEnt )
	{
		...
		if ( pPhysics )
			pPhysics->GetVelocityAtPoint( tr.endpos, &vRelativeVel );
		else
			tr.m_pEnt->GetVelocity( &vRelativeVel, NULL );

		qRelativeVel = tr.m_pEnt->GetLocalAngularVelocity();
	}
	else
	{
		vRelativeVel = vec3_origin;
		qRelativeVel = QAngle( 0, 0, 0 );
	}
...
float CWeaponPortalgun::FirePortal( bool bPortal2, Vector *pVector /*= 0*/, bool bTest /*= false*/ )
{
	...
	Vector vRelativeVel;
	QAngle qRelativeVel;
	float fPlacementSuccess = TraceFirePortal( bPortal2, vTraceStart, vDirection, tr, vRelativeVel, qRelativeVel, vFinalPosition, qFinalAngles, ePlacedBy, bTest );
	...
	if ( !bTest )
	{
		...
		float fDelay = vTracerOrigin.DistTo( tr.endpos ) / ( ( bPlayer ) ? ( sv_portal_blast_speed.GetFloat() ) : ( BLAST_SPEED_NON_PLAYER ) );

		//buffer to ensure no clipping
		vFinalPosition += vRelativeVel * fDelay;
		qFinalAngles += qRelativeVel * fDelay;
		...
		pPortal->SetContextThink( &CProp_Portal::PlacementThink, gpGlobals->curtime + fDelay, s_pDelayedPlacementContext ); 
		pPortal->m_vDelayedPosition = vFinalPosition;
		pPortal->m_hPlacedBy = this;
		*/
		CFlyingPortal *pNewPortal = CFlyingPortal::Create( vTraceStart, vDirection, ( ( bPlayer ) ? ( sv_portal_blast_speed.GetFloat() ) : ( BLAST_SPEED_NON_PLAYER ) ), m_iPortalLinkageGroupID, bPortal2 );
	}
