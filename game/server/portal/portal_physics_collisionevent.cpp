...
int CPortal_CollisionEvent::ShouldCollide( IPhysicsObject *pObj0, IPhysicsObject *pObj1, void *pGameData0, void *pGameData1 )
{
	...
	if( bShadowClonesInvolved )
	{
		...
		//CPhysicsShadowClone *pShadowClones[2] = { (CPhysicsShadowClone *) pEntities[0], (CPhysicsShadowClone *) pEntities[1] };
	...
	if ( pGameData0 != pGameData1 )
	{
		...
		for ( int i = 0; i != 2; ++i )
		{
			if ( ( pSimulators[i] != NULL ) && CPhysicsShadowClone::IsShadowClone( pEntities[i] ) )
			{
				...
				unsigned int iCloneIndex = pClone->entindex();
				...
				CPSCollisionEntity::IsPortalSimulatorCollisionEntity( pEntities[ 1 ] ) )
			{
				...
				for( int i = 0; i != 2; ++i )
				{
					if( bStatic[i] || CPSCollisionEntity::IsPortalSimulatorCollisionEntity( pEntities[ i ] ) )
					{
						...
						//code to allow entities touched by portal to not collide with world
						...
						else
						{
							...
							if ( pSimulator_Static && ( pSimulator_Static != pSimulator_Entity ) /*&& !PortalStuck*/ )
			...
			else
			{
				...
				for( int i = 0; i != 2; ++i )
				{
					if( pSimulators[i] )
					{
						...
						if ( ( pSimulators[i]->m_DataAccess.Simulation.Dynamic.EntFlags[pEntities[1 - i]->entindex()] & PSEF_CLONES_ENTITY_FROM_MAIN ) == 0 /*&& !PortalStuck*/ ) //not cloned from main
							return 0;
					}
				}
			}
		...
		int iPreview = BaseClass::ShouldCollide( pObj0, pObj1, pGameData0, pGameData1 );
		if ( iPreview )
		{
			for ( int i = 0; i < 2; ++i )
			{
