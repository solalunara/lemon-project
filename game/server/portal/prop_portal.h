...
public:
	...
	CNetworkVar( Vector, m_vPortalVelocity );
	CNetworkHandle( CBaseEntity, m_hRelativeEntity );
	...
	COutputEvent	m_rCascade;					// Output for when a portal is inside another portal
	COutputEvent	m_PortalStuck;				// Output for when you can't travel through the portal because the end is touching the ground
	...
	void					CheapUpdatePortalThink( void );
	...
	void					UpdatePortalLinkage( bool bNewPlace );
